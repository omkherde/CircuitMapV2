...
export const alzheimersDemo: TraceEvent[] = [
  ...
  },
  {
    ...
    report_sections: {
      ...
      disease_anatomy: "Alzheimer's disease atrophy pattern from meta-analytic neuroimaging:\n• Hippocampus: highest atrophy (0.92)\n• Entorhinal cortex: severe atrophy (0.88)\n• Temporal lobe: moderate atrophy (0.76)\n\nPattern consistent with Braak staging of neurofibrillary tangle progression.",
      ...
      recommendations: "1. Evaluate BBB penetration and CNS pharmacokinetics\n2. Assess selectivity against related HMT enzymes\n3. Test in preclinical AD models for cognitive endpoints\n4. Consider prodrug strategies if CNS exposure is limited",
      references: "1. ChEMBL Database (CHEMBL1234567)\n2. Allen Human Brain Atlas (human.brain-map.org)\n3. Graff J et al. Nature 2012;483(7388):222-6\n4. Peleg S et al. Science 2010;328(5979):753-6",
    },
    timestamp: Date.now(),
  },
  {
    type: 'pdf_ready',
    pdf_url: '/api/report/demo-alzheimers/download',
...
export const schizophreniaDemo: TraceEvent[] = [
  {
    type: 'agent_thought',
    content: "Analyzing Clozapine's target profile for schizophrenia validation...",
    timestamp: Date.now(),
  },
  {
    type: 'tool_call',
    tool: 'resolve_target',
    input: { query: 'Clozapine', query_type: 'name' },
    timestamp: Date.now(),
  },
  {
    type: 'tool_result',
    tool: 'resolve_target',
    summary: 'Primary targets: DRD2 (Ki=160nM), 5-HT2A (Ki=12nM), Multiple secondary targets',
    timestamp: Date.now(),
  },
  {
    type: 'confidence_update',
    dimension: 'target_resolution',
    level: 'HIGH',
    rationale: 'Clozapine has well-characterized polypharmacology with established binding affinities.',
    timestamp: Date.now(),
  },
  {
    type: 'done',
...
export const depressionDemo: TraceEvent[] = [
  {
    type: 'agent_thought',
    content: "Investigating ketamine's mechanism for treatment-resistant depression...",
    timestamp: Date.now(),
  },
  {
    type: 'tool_call',
    tool: 'resolve_target',
    input: { query: 'Ketamine', query_type: 'name' },
    timestamp: Date.now(),
  },
  {
    type: 'tool_result',
    tool: 'resolve_target',
    summary: 'Primary target: NMDA receptor (GRIN1/GRIN2A/GRIN2B) | Ki=0.5-1µM',
    timestamp: Date.now(),
  },
  {
    type: 'confidence_update',
    dimension: 'target_resolution',
    level: 'HIGH',
    rationale: 'NMDA receptor antagonism well-established. Rapid antidepressant mechanism under active investigation.',
    timestamp: Date.now(),
  },
  {
    type: 'done',
